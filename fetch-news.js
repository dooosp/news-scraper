...
async function fetchAllNews() {
    ...
    console.log(`🔥 HOT 뉴스: ${hotCount}개 (2개 이상 소스에서 보도)`);
...
async function saveResults(headlines) {
    ...
    return { archiveFilePath, todayFilePath, pptFilePath };
}
...
fetchAllNews()
    .then(async (headlines) => {
        ...
        headlines.forEach((h, i) => {
            const hot = h.isHot ? '🔥 [HOT] ' : '';
            console.log(`${i + 1}. ${hot}${h.title}`);
            console.log(`   출처: ${h.sources.join(', ')}\n`);
        });

        await saveResults(headlines);
